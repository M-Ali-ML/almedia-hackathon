...
export function PreAnalysisOverlay({ batch, onClose }: { batch: BatchResult; onClose: () => void }) {
  ...
  const findingCount = batch.findings.length
  const ruledOutCount = batch.ruled_out?.length ?? 0
        ...
        <header className="flex items-start justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-500">
              Source-grounded company context and the ingested data coverage behind the deterministic
              audit checks.
            </p>
          </div>
          <button
        ...
        <div className="overflow-y-auto px-6 py-5">
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="space-y-7">
              <section>
                <div className="mb-3">
                  <h3 className="text-sm font-bold text-slate-900">Analysis outcome</h3>
                  <p className="mt-0.5 text-xs text-slate-500">
                    Deterministic checks surface candidates; the auditor agent then confirms or rules
                    out each one, and a verifier pass re-derives every finding independently.
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <p className="text-xs font-medium text-slate-500">Confirmed findings</p>
                    <p className="mt-1 text-2xl font-bold text-slate-900">{findingCount}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <p className="text-xs font-medium text-slate-500">Ruled out (decoys)</p>
                    <p className="mt-1 text-2xl font-bold text-slate-900">{ruledOutCount}</p>
                  </div>
                </div>
              </section>
